 ...
 * 1|WR|WN|WB|WQ|WK|WB|WN|WR|1 +--+--+--+--+--+--+--+--+ a b c d e f g h
 *
 */
public class MoveValidator {
	 ...
	 *            nhận di chuyển
	 * @param debug
	 * @return true if di chuyển hợp lệ, false di chuyên k hợp lệ
	 */
	public boolean isMoveValid(Move move, boolean debug) {
		...
		if (sourcePiece.getColor() == Piece.COLOR_WHITE
			...
			log("it's not your turn: " + "pieceColor=" + Piece.getColorString(sourcePiece.getColor()) + "gameState="
	...
	private boolean isValidBishopMove(int sourceRow, int sourceColumn, int targetRow, int targetColumn) {
		// Quân tượng có thể di chuyển theo đường chéo
		// over other pieces.

		// vị trí mục tiêu cụ thể?
		if (isTargetLocationFree() || isTargetLocationCaptureable()) {
			// ok
		} else {
			// vị trí mục tiêu không free và k chụp đc
			return false;
		}

		boolean isValid = false;
		// kiểm tra xem đường dẫn đến đích có phải đường chéo k?
		int diffRow = targetRow - sourceRow;
		int diffColumn = targetColumn - sourceColumn;

		if (diffRow == diffColumn && diffColumn > 0) {
			// di chuyển theo đường chéo lên bêm phải
			...
			// Di chuyển theo đường chéo xuống bên phải
			isValid = !arePiecesBetweenSourceAndTarget(sourceRow, sourceColumn, targetRow, targetColumn, -1, +1);

		} else if (diffRow == diffColumn && diffColumn < 0) {
			// Di chuyển theo đường chéo xuống bên trái
			isValid = !arePiecesBetweenSourceAndTarget(sourceRow, sourceColumn, targetRow, targetColumn, -1, -1);

		} else if (diffRow == -diffColumn && diffColumn < 0) {
			// di chuyển theo đường chéo lên bên trái
			isValid = !arePiecesBetweenSourceAndTarget(sourceRow, sourceColumn, targetRow, targetColumn, +1, -1);

		} else {
			// không di chuyển theo đường chéo
			isValid = false;
		}
		return isValid;
	}
	...
	private boolean isValidQueenMove(int sourceRow, int sourceColumn, int targetRow, int targetColumn) {
		// Nữ hoàng kết hợp sức mạnh của rook và Bishop và có thể di chuyển bất
		// kỳ ô nào
		// theo dường chéo hoặc ngang nhưng k thể nhảy thành phần
		//
	...
	private boolean isValidPawnMove(int sourceRow, int sourceColumn, int targetRow, int targetColumn) {
		...
		boolean isValid = false;
		// quân tốt có thể di chuyển về phía trước 1 hoặc 2 ô nếu ở ô bắt đầu
		// 1 ô nếu k phải ô bắt đầu
		if (isTargetLocationFree()) {
			if (sourceColumn == targetColumn) {
				// same column
				if (sourcePiece.getColor() == Piece.COLOR_WHITE && sourcePiece.getRow() != Piece.ROW_2) {
					// white
			...
			// ăn quân của đối thủ nếu đường chéo có 1 quân
	...
	private boolean isValidKnightMove(int sourceRow, int sourceColumn, int targetRow, int targetColumn) {
		// The knight moves to any of the closest squares which are not on the
		// same rank,
		// file or diagonal, thus the move forms an "L"-shape two squares long
		// and one
		// square wide. The knight is the only piece which can leap over other
		// pieces.
	...
	public static void main(String[] args) {
		...
		isValid = mo.isValidPawnMove(sourceRow, sourceColumn, targetRow, targetColumn);
	}
